...
class SeHGNN_mag(nn.Module):
    ...
    def forward(self, batch,
                layer_feats_dict, label_emb):
        batch = self.encoder(batch)
        feats_dict = batch.x_dict
        ...
        tgt_feat = self.input_drop(batch[task].x)
        B = batch.num_nodes_dict[task]
        # tgt_feat = self.input_drop(feats_dict[task])
