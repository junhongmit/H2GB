...
from torch_geometric.nn import (Sequential, Linear, HeteroConv, SAGEConv, GINEConv, GATConv, HANConv, \
...
@register_network('HeteroGNNEdgeModel')
class HeteroGNNEdgeModel(torch.nn.Module):
    def __init__(self, dim_in, dim_out, data):
        ...
        for i in range(cfg.gnn.layers_mp):
            ...
            if cfg.gnn.layer_type == 'RGraphSAGE':
                ...
                conv = HeteroConv({
                        edge_type: GINEConv(mlp, edge_dim=cfg.gnn.dim_inner) for edge_type in self.metadata[1]
                })
                ...
                conv = HANConv(in_channels=cfg.gnn.dim_inner, out_channels=cfg.gnn.dim_inner, metadata=self.metadata,
                           heads=cfg.gnn.attn_heads, group=cfg.gnn.agg, dropout=cfg.gnn.dropout)
        ...
        self.post_mp = GNNHead(cfg.gnn.dim_inner, dim_out, data)


    def forward(self, batch):
        batch = self.encoder(batch)

        x_dict, edge_index_dict, edge_attr_dict = batch.collect('x'), \
                                                  batch.collect('edge_index'), \
                                                  batch.collect('edge_attr')
