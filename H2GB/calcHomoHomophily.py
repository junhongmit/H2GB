...
def calcHomophily(data: HeteroData, task_entity: str = None, chunk_size: int = 200):
    ...
    """
    device = data[task_entity].y.device
    data = data.cpu().to_homogeneous()

    label = data.y.to(device)
    label = label.nan_to_num(nan=-1)
    # device = label.device
    ...
    print(f"Node Homophily:{homophily(edge_index_1, label, method='node')}")
